$('[name="ewallet"]').on('change', function() {
	...
	if($(this).prop('checked')) {
		...
		$('[name="adjust"]').val(-Math.abs(Deduction.toFixed(2)))
	} else {
...
function cal() {
	...
	var subtotal = 0;
	$.each(record, function(k, v) {
		if ($('input[name="unpaid['+v+'][amount]"]').val() != null) {
	...
	if (discount_type == '$'){
		total = (subtotal-discount+adjust);
	}
	else{
		total = (subtotal-(subtotal*discount/100)+adjust);
	}
			...
			success: function(data) {
				...
				<tr class="item`+i+`">
					<td>
						...
						<textarea class="form-control mt-2" onclick="this.select();" name="item[`+i+`][remark]" rows="4">`+$('[name="'+type+'-remark"]').val()+`</textarea>
						<div class="mt-2">
