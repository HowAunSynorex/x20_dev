				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group row">
							...
							<div class="col-md-9">
								<select class="form-control select2" name="teacher" onchange="document.querySelector('.search-form').submit()" required>
									<option value="">-</option>
						...
						<div class="form-group row">
							...
							<div class="col-md-9">
								<select class="form-control select2" name="class" onchange="document.querySelector('.search-form').submit()" required>
									<option value="">-</option>
						...
						<div class="form-group row">
							<label class="col-form-label col-md-3 text-danger">Sub Class</label>
							<div class="col-md-9">
								<select class="form-control select2" name="sub_class" required>
									<option value="">-</option>
									<?php foreach($sub_class as $e) { ?>
										<option value="<?php echo $e['class_id']; ?>" <?php if(isset($_GET['sub_class'])) { if($e['class_id'] == $_GET['sub_class']) { echo 'selected'; } } ?>><?php echo $e['day'] . ' ' . $e['time_range'] . ' ' . $e['class_title'] . ' ' . $e['class_subtitle']; ?></option>
									<?php } ?>
								</select>
							</div>
						</div>
									...
									if(check_user_availability($e['user'])) { $i++;
										...
										<tr>
											...
											for($j=1; $j<=$days_in_month; $j++) {
												...
												$check_class = $this->log_join_model->list('class_timetable', branch_now('pid'), [
												...
												if(!empty($check_class) && strtotime($_GET['month'].'-'. $j) >= strtotime($join_date)) {
													// $x=time().rand(11,99);
													$dayList .= $j."+";
													if(count(
